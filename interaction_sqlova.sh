...
then
    ...
    python3 interaction_sqlova.py --data ${DATA_NAME} --num_options ${NUM_OP} --err_detector ${ED} \
        --structure ${STRUCTURE} --output_path SQLova_model/user_study/records_${OUTPUT_PATH}.json \
        --passes 1 --dropout 0.0 --temperature ${TEMP} --real_user \
        #> SQLova_model/user_study/records_${OUTPUT_PATH}.output 2>&1 &
