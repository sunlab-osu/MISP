...
class UtteranceItem():
    ...
    def gold_query_weights(self): # Added by Ziyu: learning weights
        try:
            weights = self.interaction.utterances[self.utterance_index].gold_query_weights
        except:
            weights = [1.0] * (len(self.interaction.utterances[self.utterance_index].gold_query_to_use) + 1) #EOS
        return weights
