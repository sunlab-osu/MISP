...
def write_and_evaluate(postprocess_sqls, db_path, table_schema_path, gold_path, dataset, close_verbal=False):
  db_list = []
  ...
  if dataset == 'spider':
    ...
    # TODO: (Ziyu) evaluation.py has been translated to its py3 version. Need double check.
    command = 'python eval_scripts/evaluation.py --db {} --table {} --etype match --gold {} --pred {}'.format(
      db_path, table_schema_path, gold_path, os.path.abspath(output_file))
    if close_verbal:
      command += ' --close_verbal'
...
if __name__ == '__main__':
  ...
  command = write_and_evaluate(postprocess_sqls, db_path, table_schema_path, gold_path, args.dataset,
                               close_verbal=args.close_verbal)
