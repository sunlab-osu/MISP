"""
Adapted to support bayesian dropout
"""
import torch
...
def run_lstm(lstm, inp, inp_len, hidden=None, dropout_rate=0.):
    ...
    if dropout_rate > 0.:
        # ret_s of shape [batch_size, seq_len, hid_size]
        mask_s = torch.FloatTensor(ret_s.size()[-1]).view(1, 1, -1)\
            .fill_(1. - dropout_rate).bernoulli().div_(1. - dropout_rate)
        if ret_s.is_cuda:
            mask_s = mask_s.cuda()
        ret_s.data = ret_s.data * mask_s

    return ret_s, ret_h


def col_name_encode(name_inp_var, name_len, col_len, enc_lstm, dropout_rate=0.):
    #Encode the columns.
    #The embedding of a column name is the last state of its LSTM output.
    name_hidden, _ = run_lstm(enc_lstm, name_inp_var, name_len, dropout_rate=dropout_rate)
