...
if __name__ == '__main__':
    ...
    parser.add_argument('--baseline', action='store_true', 
            help='If set, then train Seq2SQL model; default is SQLNet_model model.')
    parser.add_argument('--train_emb', action='store_true',
            help='Train word embedding for SQLNet_model(requires pretrained model).')
    ...
    if args.baseline:
        ...
        assert not args.rl, "SQLNet_model can\'t do reinforcement learning."
    optimizer = torch.optim.Adam(model.parameters(),
            lr=learning_rate, weight_decay = 0)
    print("## Model built up!")
    print("## GPU usage: available {}, current_device {}".format(
