...
from MISP_SQL.utils import SELECT_COL, SELECT_AGG, WHERE_COL, WHERE_OP, WHERE_ROOT_TERM, ANDOR, GROUP_COL, GROUP_NHAV,\
    HAV_COL, HAV_AGG, HAV_OP, HAV_ROOT_TERM, ORDER_COL, ORDER_AGG, ORDER_DESC_ASC_LIMIT, IUEN, OUTSIDE, END_NESTED
                    ...
                    if isinstance(vet, tuple) and vet[0] == "root":
                        ...
                        if vet[1] == "nested" or vet[1] == "original":
                            ...
                            for label_idx in sorted_label_indices:
                                ...
                                new_hyp.tag_seq.append((IUEN, (new_hyp.label, label_idx),
                                                        score_prob[label_idx], new_hyp.dec_seq_idx))
                        ...
                        if len(hyp.dec_prefix):
                            ...
                            if bool_collect_choices:
                                col_num_cols_pair = [(1, [col_idx]) for col_idx in sorted_cols[:beam_size]]
                            else:
                        ...
                        for col_num, cols in col_num_cols_pair:
                            ...
                            original_col_names = [index_to_column_name(col, tables) for col in cols] #index_to_column_name_original
                            ...
                            for col, col_name, original_col_name in zip(cols, col_names, original_col_names):
                                if vet[1] == "where":
                                    ...
                                    step_hyp.tag_seq.append((WHERE_COL, original_col_name, col_score_prob[col],
                                    ...
                                    if vet[1] == "select":
                                        ...
                                        assert vet[1] == "having"
                                        tag = HAV_COL
                                    step_hyp.tag_seq.append((tag, original_col_name,
                                                             col_score_prob[col], step_hyp.dec_seq_idx))
                                    step_hyp.prob_stack.push(("agg", vet[1], original_col_name))
                                elif vet[1] == "groupBy":
                                    step_hyp.history[0].append(col_name)
                                    step_hyp.tag_seq.append((GROUP_COL, original_col_name, col_score_prob[col],
                                                             step_hyp.dec_seq_idx))
                            ...
                            if bool_collect_choices and len(hyp.dec_prefix) == 0: # no need to continue
                                step_hypotheses.append(step_hyp)
                                continue
                        ...
                        if len(hyp.dec_prefix):
                            ...
                            if avoid_items is not None and hyp.dec_seq_idx in avoid_items:
                                ...
                                if 0 in avoid_aggs: # none_agg
                                    sorted_agg_num_list.remove(0)
                                avoid_aggs = [agg-1 for agg in avoid_aggs] # NOTE: agg_idx in avoid/confirmed_items is its corresponding here + 1
                            ...
                            if confirmed_items is not None and hyp.dec_seq_idx in confirmed_items:
                                ...
                                fixed_aggs = list(agg - 1 for agg in confirmed_items[hyp.dec_seq_idx])
                            ...
                            if bool_collect_choices:
                                if 0 in sorted_agg_num_list:
                                    agg_num_idxs_pairs = [(1, [agg]) for agg in sorted_agg_idxs[:(beam_size-1)]] + [(0, [])]
                                else:
                                    agg_num_idxs_pairs = [(1, [agg]) for agg in sorted_agg_idxs[:beam_size]]
                            else:
                        ...
                        for agg_num, agg_idxs in agg_num_idxs_pairs:
                            ...
                            if len(agg_idxs) > 0:
                                ...
                                if vet[1] not in ("having", "orderBy"):
                                    ...
                                    step_hyp.tag_seq.append((SELECT_AGG, col_name, (AGG_OPS[agg_idxs[0]], agg_idxs[0]+1),
                                                             agg_score_prob[agg_idxs[0]], step_hyp.dec_seq_idx))
                                elif vet[1] == "orderBy":
                                    step_hyp.stack.push(("des_asc", vet[2], AGG_OPS[agg_idxs[0]]))  # DEBUG-ed 20180817
                                    step_hyp.prob_stack.push(("des_asc", vet[2],
                                                              (col_name, (AGG_OPS[agg_idxs[0]], agg_idxs[0]+1))))
                                    step_hyp.tag_seq.append((ORDER_AGG, col_name, (AGG_OPS[agg_idxs[0]], agg_idxs[0]+1),
                                                             agg_score_prob[agg_idxs[0]],
                                                             step_hyp.dec_seq_idx))
                                else:  # having
                                    step_hyp.stack.push(("op", "having", vet[2], AGG_OPS[agg_idxs[0]]))
                                    step_hyp.prob_stack.push(("op", "having", vet[2],
                                                              (col_name, (AGG_OPS[agg_idxs[0]], agg_idxs[0]+1))))
                                    step_hyp.tag_seq.append((HAV_AGG, col_name, (AGG_OPS[agg_idxs[0]], agg_idxs[0]+1),
                                                             agg_score_prob[agg_idxs[0]],
                                                             step_hyp.dec_seq_idx))
                            for agg in agg_idxs[1:]:
                                ...
                                if vet[1] not in ("having", "orderBy"):
                                    ...
                                    step_hyp.tag_seq.append((SELECT_AGG, col_name, (AGG_OPS[agg], agg+1),
                            ...
                            if len(agg_idxs) == 0:
                                ...
                                if vet[1] in {"orderBy", "having", "select"}:
                                    if vet[1] == "orderBy":
                                        ...
                                        tag = HAV_AGG
                                    step_hyp.tag_seq.append((tag, col_name, ("none_agg", 0),
                                ...
                                if vet[1] not in ("having", "orderBy"):
                                    ...
                                    step_hyp.prob_stack.push(("des_asc", vet[2], (col_name, ("none_agg", 0))))
                                else: #having
                                    step_hyp.stack.push(("op", "having", vet[2], "none_agg"))
                                    step_hyp.prob_stack.push(("op", "having", vet[2], (col_name, ("none_agg", 0))))
                        ...
                        if len(hyp.dec_prefix):
                            ...
                            if bool_collect_choices:
                                op_num_ops_pairs = [(1, [op]) for op in sorted_ops[:beam_size]]
                            else:
                        ...
                        for op_num, ops in op_num_ops_pairs:
                            ...
                            if op_num > 0:
                                ...
                                if vet[1] == "having":
                                    ...
                                    step_hyp.prob_stack.push(("root_teminal", vet[2], vet[3], vet[1], col_agg,
                                                              (NEW_WHERE_OPS[ops[0]], ops[0])))
                                    step_hyp.tag_seq.append((HAV_OP, col_agg, (NEW_WHERE_OPS[ops[0]], ops[0]),
                                                             op_score_prob[ops[0]], step_hyp.dec_seq_idx))
                                else: #where
                                    step_hyp.stack.push(("root_teminal", vet[2], ops[0]))
                                    step_hyp.prob_stack.push(("root_teminal", vet[2], vet[1], col_agg,
                                                              (NEW_WHERE_OPS[ops[0]], ops[0])))
                                    step_hyp.tag_seq.append((WHERE_OP, col_agg, (NEW_WHERE_OPS[ops[0]], ops[0]),
                                                             op_score_prob[ops[0]], step_hyp.dec_seq_idx))
                            ...
                            for op in ops[1:]:
                                ...
                                if vet[1] == "having":
                                    ...
                                    step_hyp.prob_stack.push(("root_teminal", vet[2], vet[3], vet[1], col_agg,
                                                              (NEW_WHERE_OPS[op], op)))
                                    step_hyp.tag_seq.append((HAV_OP, col_agg, (NEW_WHERE_OPS[op], op),
                                                             op_score_prob[op], step_hyp.dec_seq_idx))
                                else:
                                    step_hyp.stack.push(("root_teminal", vet[2], op))
                                    step_hyp.prob_stack.push(("root_teminal", vet[2], vet[1], col_agg,
                                                              (NEW_WHERE_OPS[op], op)))
                                    step_hyp.tag_seq.append((WHERE_OP, col_agg, (NEW_WHERE_OPS[op], op),
                                                             op_score_prob[op], step_hyp.dec_seq_idx))
                        ...
                        for label_idx in label_indices:
                            ...
                            if step_hyp.label == "root":
                                ...
                                step_hyp.tag_seq.append((tag, col_agg, op, "root", score_prob[label_idx], step_hyp.dec_seq_idx))
                                step_hyp.prob_stack.push(("root", step_hyp.current_sql[step_hyp.kw][-1]))
                                step_hyp.dec_seq_idx += 1
                            else:
                                step_hyp.current_sql[step_hyp.kw].append("terminal")
                                step_hyp.tag_seq.append((tag, col_agg, op, "terminal", score_prob[label_idx], step_hyp.dec_seq_idx))
                                step_hyp.dec_seq_idx += 1
