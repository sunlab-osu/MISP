...
class DesAscLimitPredictor(nn.Module):
    def __init__(self, N_word, N_h, N_depth, gpu, use_hs, dr=0.3, temperature=False):
        ...
        if temperature:
            self.T = nn.Parameter(torch.FloatTensor([1.])) # dat_out
        else:
            self.T = 1.0
    ...
    def forward(self, q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, gt_col):
        ...
        # dat_score: (B, 4)
        dat_score = self.dat_out(self.dat_out_q(q_weighted) + int(self.use_hs)* self.dat_out_hs(hs_weighted) +
                                 self.dat_out_c(col_emb)) / self.T
