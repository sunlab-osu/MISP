...
class OpPredictor(nn.Module):
    def __init__(self, N_word, N_h, N_depth, gpu, use_hs, dr=0.3, temperature=False):
        ...
        if temperature:
            self.T1 = nn.Parameter(torch.FloatTensor([1.])) # op_num_out
            self.T2 = nn.Parameter(torch.FloatTensor([1.])) # op_out
        else:
            self.T1 = self.T2 = 1.
    ...
    def forward(self, q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len, gt_col):
        ...
        # op_num_score: (B, 2)
        op_num_score = self.op_num_out(self.op_num_out_q(q_weighted_num) +
                                       int(self.use_hs)* self.op_num_out_hs(hs_weighted_num) +
                                       self.op_num_out_c(col_emb)) / self.T1
        ...
        # op_score: (B, 10)
        op_score = self.op_out(self.op_out_q(q_weighted) + int(self.use_hs)* self.op_out_hs(hs_weighted) +
                               self.op_out_c(col_emb)) / self.T2
