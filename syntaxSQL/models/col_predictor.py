...
class ColPredictor(nn.Module):
    def __init__(self, N_word, N_h, N_depth, gpu, use_hs, dr=0.3, temperature=False):
        ...
        if temperature:
            self.T1 = nn.Parameter(torch.FloatTensor([1.])) # col_num_out
            self.T2 = nn.Parameter(torch.FloatTensor([1.])) # col_out
        else:
            self.T1 = self.T2 = 1.0
    ...
    def forward(self, q_emb_var, q_len, hs_emb_var, hs_len, col_emb_var, col_len, col_name_len):
        ...
        col_num_score = self.col_num_out(self.col_num_out_q(q_weighted_num) +
                                         int(self.use_hs)* self.col_num_out_hs(hs_weighted_num)) / self.T1
        ...
        col_score = self.col_out(self.col_out_q(q_weighted) + int(self.use_hs)* self.col_out_hs(hs_weighted) +
                                 self.col_out_c(col_enc)).view(B,-1) / self.T2
