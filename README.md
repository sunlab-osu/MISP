...
Our MISP-SQL framework is under the folder "[MISP_SQL](MISP_SQL/)", with [`agent.py`](MISP_SQL/agent.py) being the implemented MISP agent. The interactive semantic parsing session is implemented in the function [`interactive_parsing_session`](https://github.com/LittleYUYU/MISP-private/blob/multichoice_q/MISP_SQL/agent.py#L34). 
As an overview:
