...
then
    ...
    python interaction_SQLNet.py --ca --train_emb --data ${DATA_NAME} --num_options ${NUM_OP} --err_detector ${ED} \
        --structure ${STRUCTURE} --output_path SQLNet_model/user_study/records_${OUTPUT_PATH}.json \
        --passes 1 --dropout 0.0 --temperature ${TEMP} --real_user \
        #> SQLNet_model/user_study/records_${OUTPUT_PATH}.output
else
    if [ ${NUM_PASSES} -gt 1 ]
    then
