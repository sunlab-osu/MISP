...
from syntaxSQL.supermodel import SuperModel, BasicHypothesis
from syntaxSQL.ISQL import ISQLSyntaxSQL
from syntaxSQL.err_detector import ErrorDetectorProbability, ErrorDetectorEvaluatorSyntaxSQL, ErrorDetectorBayDropout
from syntaxSQL.user_simulator import UserSim, RealUsersyntaxSQL
...
def interaction(agent, data, output_path, bool_interaction=True):
    ...
    for item_idx, item in enumerate(data):
        ...
        hyp = agent.decode(item, bool_verbal=False)[0]
        ...
        hardness, bool_err, exact_score, partial_scores = agent.evaluation(item, hyp)
        ...
        hyp, bool_exit = agent.error_detection(item, item['sql'], hyp, bool_verbal=False)
    ...
    # results.append({'necessary_q': agent.necessary_q_counter, 'wasteful_q': agent.waste_q_counter})
...
def real_user_interaction(agent, data, output_path):
    ...
    while st < len(data):
        ...
        agent.user_sim.show_table(item['db_id'])  # print table
        ...
        # # TODO: debug only
        # print("(Debug) True SQL: {}".format(item['query']))
        ...
        start_time = time.time()
        # init decode
        init_hyp = agent.decode(item, bool_verbal=False)[0]

        hyp, bool_exit = agent.error_detection(item, item['sql'], init_hyp, bool_verbal=False)
...
if __name__ == '__main__':
    ...
    model.multi_sql.load_state_dict(torch.load("{}/{}multi_sql_models.dump".format(args.models, prefix)))
    model.key_word.load_state_dict(torch.load("{}/{}keyword_models.dump".format(args.models, prefix)))
    model.col.load_state_dict(torch.load("{}/{}col_models.dump".format(args.models, prefix)))
    model.op.load_state_dict(torch.load("{}/{}op_models.dump".format(args.models, prefix)))
    model.agg.load_state_dict(torch.load("{}/{}agg_models.dump".format(args.models, prefix)))
    model.root_teminal.load_state_dict(torch.load("{}/{}root_tem_models.dump".format(args.models, prefix)))
    model.des_asc.load_state_dict(torch.load("{}/{}des_asc_models.dump".format(args.models, prefix)))
    model.having.load_state_dict(torch.load("{}/{}having_models.dump".format(args.models, prefix)))
    model.andor.load_state_dict(torch.load("{}/{}andor_models.dump".format(args.models, prefix)))

    print "Creating ISQL agent..."
    question_generator = QuestionGenerator()
    err_evaluator = ErrorDetectorEvaluatorSyntaxSQL()
    # user_simulator = RealUser(err_evaluator) if args.real_user else UserSim(err_evaluator)
    if args.real_user:
        user_simulator = RealUsersyntaxSQL(err_evaluator, get_table_dict("syntaxSQL/data/tables.json"))
    else:
    ...
    agent = ISQLSyntaxSQL(model, error_detector, question_generator, user_simulator, num_options,
                          bool_structure_rev=args.structure, num_passes=args.passes, dropout_rate=args.dropout)

    if args.real_user:
        real_user_interaction(agent, data, args.output_path)
    else:
        interaction(agent, data, args.output_path, bool_interaction=True)
