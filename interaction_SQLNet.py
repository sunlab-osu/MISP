...
from SQLNet_model.sqlnet.model.sqlnet import SQLNet
import numpy as np
import time, os
import argparse
import random
...
from SQLNet_model.agent import Agent
from SQLNet_model.world_model import WorldModel
from SQLNet_model.error_detector import *
from SQLNet_model.environment import UserSim, RealUser, ErrorEvaluator
...
def real_user_interaction(user, agent, sql_data, table_data, db_path, save_path):
    ...
    while st < len(sql_data):
        ...
        user.show_table(table_ids[0])  # print table
        ...
        init_hyp = agent.world_model.decode(input_item, bool_verbal=False)[0]

        hyp, bool_exit = agent.interactive_parsing_session(user, input_item, query_gt[0], init_hyp, bool_verbal=False)
        print("\nPredicted SQL: {}\n".format(hyp.sql))
...
def interaction(user, agent, sql_data, table_data, db_path, wikisql_sample_ids=None, bool_interaction=True):
    engine = DBEngine(db_path)
    ...
    while st < len(sql_data):
        ...
        hyp = agent.world_model.decode(input_item, bool_verbal=False)[0]
        ...
        per_qm_one_acc_num, per_qm_tot_acc_num, per_exe_tot_acc_num = agent.evaluation(
            raw_data, [hyp.sql_i], query_gt, table_ids, engine)
        print("qm correct: {}, exe correct: {}".format(per_qm_tot_acc_num, per_exe_tot_acc_num))
        ...
        hyp, bool_exit = agent.interactive_parsing_session(user, input_item, query_gt[0], hyp, bool_verbal=False)
...
if __name__ == '__main__':
    ...
    if args.real_user:
        user = RealUser(error_evaluator, test_table_data)
    else:
    ...
    if args.train_emb:
        ...
        model.agg_pred.load_state_dict(torch.load(agg_m, map_location=device))
        print "Loading from %s"%sel_m
        model.sel_pred.load_state_dict(torch.load(sel_m, map_location=device))
        print "Loading from %s"%cond_m
        model.cond_pred.load_state_dict(torch.load(cond_m, map_location=device))
        print "Loading from %s"%agg_e
        model.agg_embed_layer.load_state_dict(torch.load(agg_e, map_location=device))
        print "Loading from %s"%sel_e
        model.sel_embed_layer.load_state_dict(torch.load(sel_e, map_location=device))
        print "Loading from %s"%cond_e
        model.cond_embed_layer.load_state_dict(torch.load(cond_e, map_location=device))
    else:
    ...
    wikisql_sample_ids = None
    if args.data == 'dev':
        assert user.user_type == "sim"
        qm_one_acc, qm_tot_acc, exe_tot_acc, interaction_records = interaction(
            user, agent, val_sql_data, val_table_data, DEV_DB, wikisql_sample_ids=wikisql_sample_ids)
        ...
        assert user.user_type == "sim"
        qm_one_acc, qm_tot_acc, exe_tot_acc, interaction_records = interaction(
            user, agent, test_sql_data, test_table_data, TEST_DB)
        ...
        if user.user_type == "real":
            real_user_interaction(user, agent, sql_data, test_table_data, TEST_DB, args.output_path)
        else:
            qm_one_acc, qm_tot_acc, exe_tot_acc, interaction_records = interaction(
                user, agent, sql_data, test_table_data, TEST_DB, bool_interaction=True)
            print "Test acc_qm: %s;\n  breakdown on (agg, sel, where): %s\nTest execution acc: %s" % (
                qm_tot_acc, qm_one_acc, exe_tot_acc)
            json.dump(interaction_records, open(args.output_path, 'w'), indent=4)
