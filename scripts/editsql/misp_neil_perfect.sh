...
SUP="misp_neil_perfect"
DATA_SEED=0 # 0, 10, 100
ST=0
END=-1

OUTPUT_PATH=${SUP}_OP${NUM_OP}_ED${ED}_ITER${UPDATE_ITER}_DATASEED${DATA_SEED}
echo ${OUTPUT_PATH}
...
python3 interaction_editsql.py \
      ...
      --job="online_learning" \
      --num_options=${NUM_OP} --err_detector=${ED} --friendly_agent=0 --user="gold_sim" \
