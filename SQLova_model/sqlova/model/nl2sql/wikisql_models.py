...
# Adapted for interaction. @author: Ziyu Yao.
...
from SQLova_model.sqlova.utils.utils import topk_multi_dim
from SQLova_model.sqlova.utils.utils_wikisql import *
from MISP_SQL.utils import Hypothesis as BaseHypothesis
from MISP_SQL.utils import SELECT_COL, SELECT_AGG, WHERE_COL, WHERE_OP, WHERE_VAL, OUTSIDE
...
class Seq2SQL_v1(nn.Module):
    def __init__(self, iS, hS, lS, dr, n_cond_ops, n_agg_ops, old=False, temperature=False):
        ...
        # self.wcp = WCP(iS, hS, lS, dr, temperature=temperature)
        self.scp = SCP(iS, hS, lS, dr, temperature=temperature)
        self.sap = SAP(iS, hS, lS, dr, n_agg_ops, old=old, temperature=temperature)
        self.wnp = WNP(iS, hS, lS, dr, temperature=temperature)
        self.wcp = WCP(iS, hS, lS, dr, temperature=temperature)
        self.wop = WOP(iS, hS, lS, dr, n_cond_ops, temperature=temperature)
        self.wvp = WVP_se(iS, hS, lS, dr, n_cond_ops, old=old, temperature=temperature) # start-end-search-discriminative model
    ...
    def interaction_beam_forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, tb, nlu_t, nlu_wp_t, wp_to_wh_index, nlu,
                                 beam_size, dec_prefix, stop_step=None, avoid_items=None, confirmed_items=None,
                                 dropout_rate=0.0, bool_collect_choices=False, bool_verbal=False):
        """
        @author: Ziyu Yao
        Beam search decoding for interactive sql generation. Only support bS=1 (one example per batch).
        """
        dec_prefix = dec_prefix[::-1]
        hypotheses = [Hypothesis(dec_prefix)]
        completed_hypotheses = []
        table_name = None #tb[0]["id"]
        ...
        while True:
            ...
            for hyp in hypotheses:
                if hyp.stack.isEmpty():
                    ...
                    hyp.sql = generate_sql_q1(hyp.sql_i, tb[0])
                    if bool_verbal:
                    ...
                    if vet[0] == "sc":
                        # sc
                        s_sc = self.scp(wemb_n, l_n, wemb_hpu, l_hpu, l_hs, dropout_rate=dropout_rate)
                        prob_sc = F.softmax(s_sc, dim=-1).detach().to('cpu').numpy()[0]
                        ...
                        s_sa = self.sap(wemb_n, l_n, wemb_hpu, l_hpu, l_hs, [sc_idx], dropout_rate=dropout_rate)
                        prob_sa = F.softmax(s_sa, dim=-1).detach().to('cpu').numpy()[0]
                        ...
                        for sa_idx in sa_candidates:
                            ...
                            step_hyp.tag_seq.append((SELECT_AGG, (table_name, sc_name, sc_idx), (sa_name, sa_idx),
                        ...
                        step_hypotheses = []
                        # wn
                        s_wn = self.wnp(wemb_n, l_n, wemb_hpu, l_hpu, l_hs, dropout_rate=dropout_rate)
                        prob_wn = F.softmax(s_wn, dim=-1).detach().to('cpu').numpy()[0]

                        # wc
                        s_wc = self.wcp(wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wc=False, penalty=True,
                                        dropout_rate=dropout_rate)
                        prob_wc = torch.sigmoid(s_wc).detach().to('cpu').numpy()[0]
                        ...
                        if len(hyp.dec_prefix):
                            ...
                            if confirmed_items is not None and hyp.dec_seq_idx in confirmed_items:
                                ...
                                sorted_cols = [col for col in sorted_cols if col not in fixed_cols]
                            else:
                                fixed_cols = []

                            if bool_collect_choices:
                                col_num_cols_pair.extend([(1, [col_idx]) for col_idx in sorted_cols[:beam_size]])
                            else:
                                for col_num in sorted_col_num: #[:beam_size]
                                    if col_num == 0:
                                        ...
                                        col_num_cols_pair.extend(
                                            [(len(fixed_cols) + 1, fixed_cols + [col]) for col in sorted_cols[:beam_size]])
                                    elif beam_size == 1:
                        ...
                        for col_num, cols in col_num_cols_pair:
                            ...
                            for wc_idx in cols:
                                wc_name = tb[0]['header'][wc_idx]
                                step_hyp.tag_seq.append((WHERE_COL, (table_name, wc_name, wc_idx), prob_wc[wc_idx],
                                                         step_hyp.dec_seq_idx))
                        ...
                        s_wo = self.wop(wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn=[1], wc=[[wc_idx]]) #[1, 4, n_op]
                        prob_wo = F.softmax(s_wo, dim=-1).detach().to('cpu').numpy()[0][0]
                        ...
                        for wo_idx in wo_candidates:
                            ...
                            step_hyp.tag_seq.append((WHERE_OP, ((table_name, wc_name, wc_idx),), (wo_name, wo_idx),
                        ...
                        s_wv = self.wvp(wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn=[1], wc=[[wc_idx]], wo=[[wo_idx]]) #[1, 4, mL, 2]

                        if len(hyp.dec_prefix):
                            partial_vet, st, ed = hyp.dec_prefix.pop()
                            assert partial_vet == vet
                            prob_wv = F.softmax(s_wv, dim=-2).detach().to('cpu').numpy()[0][0] #[mL, 2]
                            pr_wvi_beam = [(st, ed)]
                            prob_wvi_beam = [prob_wv[st][0] * prob_wv[ed][1]]
                        else:
                            # pr_wvi_beam = [1, 1, n_pairs, 2]. 2 means [st, ed].
                            # prob_wvi_beam = [1, 1, n_pairs]
                            avoid_item_list = []
                            if avoid_items is not None and hyp.dec_seq_idx in avoid_items:
                                avoid_item_list = list(avoid_items[hyp.dec_seq_idx])

                            pr_wvi_beam, prob_wvi_beam = pred_wvi_se_interaction_beam(
                                1, s_wv, 100 if beam_size is None else beam_size, avoid_item_list)
                            pr_wvi_beam = pr_wvi_beam[0][0]
                            prob_wvi_beam = prob_wvi_beam[0][0]

                        for pair_idx, (st, ed) in enumerate(pr_wvi_beam):
                            if len(prob_wvi_beam) == 1:
                                step_hyp = hyp
                            else:
                                step_hyp = hyp.copy()
                            prob_pair = prob_wvi_beam[pair_idx]

                            # get wv_str
                            temp_pr_wv_str, _ = convert_pr_wvi_to_string([[(st, ed)]], nlu_t, nlu_wp_t, wp_to_wh_index, nlu)
                            val_str = merge_wv_t1_eng(temp_pr_wv_str[0][0], nlu[0])

                            step_hyp.sql_i['conds'].append([wc_idx, wo_idx, val_str])
                            step_hyp.dec_seq.append((vet, st, ed))
                            step_hyp.tag_seq.append((WHERE_VAL, ((table_name, wc_name, wc_idx),), (wo_name, wo_idx),
                                                     (st, ed, val_str), prob_pair, hyp.dec_seq_idx))
                            step_hyp.add_logprob(np.log(prob_pair))
                            step_hyp.dec_seq_idx += 1
...
class SCP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, temperature=False):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_sc=False, dropout_rate=0.):
        # Encode
        wenc_n = encode(self.enc_n, wemb_n, l_n,
                        return_hidden=False,
                        hc0=None,
                        last_only=False,
                        dropout_rate=dropout_rate)  # [b, n, dim]

        wenc_hs = encode_hpu(self.enc_h, wemb_hpu, l_hpu, l_hs,
                             dropout_rate=dropout_rate)  # [b, hs, dim]
        ...
        att_h = punish_len(att_h, torch.tensor(l_n).reshape(-1, 1, 1), device)
        ...
        if dropout_rate > 0.0:
            vec_mask = torch.FloatTensor(vec.size()[-1]).view(1, 1, -1)\
                .fill_(1. - dropout_rate).bernoulli().div_(1. - dropout_rate).to(device)
            vec.data = vec.data * vec_mask

        s_sc = self.sc_out(vec).squeeze(2) / self.T # [bS, mL_hs, 1] -> [bS, mL_hs]

        # Penalty
        mL_hs = max(l_hs)
        s_sc = punish_len(s_sc, torch.tensor(l_hs).reshape(-1, 1), device)
...
class SAP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_agg_ops=-1, old=False, temperature=False):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, pr_sc, show_p_sa=False, dropout_rate=0.):
        # Encode
        wenc_n = encode(self.enc_n, wemb_n, l_n,
                        return_hidden=False,
                        hc0=None,
        ...
        #   Penalty on blank parts
        att = punish_len(att, torch.tensor(l_n).reshape(-1, 1), device)
        # for b, l_n1 in enumerate(l_n):
        #     if l_n1 < mL_n:
        ...
        if dropout_rate > 0.0:
            c_n_mask = torch.FloatTensor(c_n.size()[-1]).view(1, -1)\
                .fill_(1. - dropout_rate).bernoulli().div_(1. - dropout_rate).to(device)
            c_n.data = c_n.data * c_n_mask

        s_sa = self.sa_out(c_n) / self.T
...
class WNP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wn=False, dropout_rate=0.):
        # Encode

        wenc_hs = encode_hpu(self.enc_h, wemb_hpu, l_hpu, l_hs,
                             dropout_rate=dropout_rate)  # [b, mL_hs, dim]
        ...
        #   Penalty
        att_h = punish_len(att_h, torch.tensor(l_hs).reshape(-1, 1), device)
        # for b, l_hs1 in enumerate(l_hs):
        #     if l_hs1 < mL_hs:
        ...
        #    Penalty
        att_n = punish_len(att_n, torch.tensor(l_n).reshape(-1, 1), device)
        # for b, l_n1 in enumerate(l_n):
        #     if l_n1 < mL_n:
...
class WCP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wc, penalty=True, dropout_rate=0.):
        # Encode
        wenc_n = encode(self.enc_n, wemb_n, l_n,
                        return_hidden=False,
                        hc0=None,
        ...
        mL_n = max(l_n)
        att = punish_len(att, torch.tensor(l_n).reshape(-1, 1, 1), device)
        ...
        if dropout_rate > 0.0:
            y_mask = torch.FloatTensor(y.size()[-1]).view(1, 1, -1)\
                .fill_(1. - dropout_rate).bernoulli().div_(1. - dropout_rate).to(device)
            y.data = y.data * y_mask

        score = self.W_out(y).squeeze(2) / self.T  # [b, hs]

        if penalty:
            score = punish_len(score, torch.tensor(l_hs).reshape(-1, 1), device)
            # for b, l_hs1 in enumerate(l_hs):
            #     score[b, l_hs1:] = -1e+10
...
class WOP(nn.Module):
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_cond_ops=3, temperature=False):
...
class WVP_se(nn.Module):
    ...
    """
    def __init__(self, iS=300, hS=100, lS=2, dr=0.3, n_cond_ops=4, old=False, temperature=False):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn, wc, wo, wenc_n=None, show_p_wv=False,
        ...
        if dropout_rate > 0.:
            vec2_mask = torch.FloatTensor(vec2.size()[-1])\
                .fill_(1. - dropout_rate).bernoulli().div_(1. - dropout_rate).to(device)
            vec2.data = vec2.data * vec2_mask

        # now make logits
        s_wv = self.wv_out(vec2) / self.T # [bS, 4, mL, 400] -> [bS, 4, mL, 2]

        # penalty for spurious tokens
        max_len = s_wv.size(-2)
        mask = (torch.arange(max_len).expand(s_wv.size()[:-1]) >= torch.tensor(l_n).reshape(-1, 1, 1)).float().unsqueeze(-1)
        mask = mask.to(device)
        s_wv = s_wv * (1 - mask) + mask * -10000000000
        # for b, l_n1 in enumerate(l_n):
        #     if l_n1 < mL_n:
...
def Loss_sw_se(s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi,
               weight_sc=None, weight_sa=None, weight_wn=None, weight_wc=None, weight_wo=None, weight_wvi=None):
    """
    ...
    """
    loss = 0
    loss += Loss_sc(s_sc, g_sc, weight_sc=weight_sc)
    loss += Loss_sa(s_sa, g_sa, weight_sa=weight_sa)
    loss += Loss_wn(s_wn, g_wn, weight_wn=weight_wn)
    loss += Loss_wc(s_wc, g_wc, weight_wc=weight_wc)
    loss += Loss_wo(s_wo, g_wn, g_wo, weight_wo=weight_wo)
    loss += Loss_wv_se(s_wv, g_wn, g_wvi, weight_wvi=weight_wvi)

    return loss

def Loss_sc(s_sc, g_sc, weight_sc=None):
    if weight_sc is not None:
        loss = torch.sum(F.cross_entropy(s_sc, torch.tensor(g_sc).to(device), reduction='none') * \
                         torch.tensor(weight_sc).to(device)) / (sum(weight_sc) + MINIMUM)
    else:
...
def Loss_sa(s_sa, g_sa, weight_sa=None):
    if weight_sa is not None:
        loss = torch.sum(F.cross_entropy(s_sa, torch.tensor(g_sa).to(device), reduction='none') * \
                         torch.tensor(weight_sa).to(device)) / (sum(weight_sa) + MINIMUM)
    else:
        loss = F.cross_entropy(s_sa, torch.tensor(g_sa).to(device))
    return loss

def Loss_wn(s_wn, g_wn, weight_wn=None):
    if weight_wn is not None:
        loss = torch.sum(F.cross_entropy(s_wn, torch.tensor(g_wn).to(device), reduction='none') *\
                         torch.tensor(weight_wn).to(device)) / (sum(weight_wn) + MINIMUM)
    else:
        loss = F.cross_entropy(s_wn, torch.tensor(g_wn).to(device))

    return loss

def Loss_wc(s_wc, g_wc, weight_wc=None):
    ...
    for b, g_wc1 in enumerate(g_wc):
        for idx, g_wc11 in enumerate(g_wc1):
            if weight_wc is None or weight_wc[b][idx] == 1.0:
                im[b, g_wc11] = 1.0
...
def Loss_wo(s_wo, g_wn, g_wo, weight_wo=None):
    ...
    for b, g_wn1 in enumerate(g_wn):
        ...
        if weight_wo is not None:
            loss += torch.sum(F.cross_entropy(s_wo1[:g_wn1], torch.tensor(g_wo1).to(device), reduction='none') *
                              torch.tensor(weight_wo[b]).to(device)) / (sum(weight_wo[b]) + MINIMUM)
        else:
            loss += F.cross_entropy(s_wo1[:g_wn1], torch.tensor(g_wo1).to(device))

    return loss

def Loss_wv_se(s_wv, g_wn, g_wvi, weight_wvi=None):
    """
    ...
    for b, g_wvi1 in enumerate(g_wvi):
        ...
        if weight_wvi is not None:
            loss += torch.sum(F.cross_entropy(s_wv[b,:g_wn1,:,0], g_st1, reduction='none') * \
                              torch.tensor(weight_wvi[b]).to(device)) / (sum(weight_wvi[b]) + MINIMUM)
        else:
        ...
        if weight_wvi is not None:
            loss += torch.sum(F.cross_entropy(s_wv[b, :g_wn1, :, 1], g_ed1, reduction='none') * \
                              torch.tensor(weight_wvi[b]).to(device)) / (sum(weight_wvi[b]) + MINIMUM)
        else:
...
def analysis_sw_se_seq(s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi,
                       conf_threshold):
    # Return a sequence NLL loss for each decision.
    # Used in regret analysis.
    loss_seq, bool_query_seq, bool_noquery_err_seq, categories = [], [], [], []

    loss_seq_sc, bool_query_seq_sc, bool_noquery_err_seq_sc = analysis_sc_seq(s_sc, g_sc, conf_threshold)
    loss_seq.extend(loss_seq_sc)
    bool_query_seq.extend(bool_query_seq_sc)
    bool_noquery_err_seq.extend(bool_noquery_err_seq_sc)
    categories.extend(['sc'] * len(loss_seq_sc))

    loss_seq_sa, bool_query_seq_sa, bool_noquery_err_seq_sa = analysis_sc_seq(s_sa, g_sa, conf_threshold)
    loss_seq.extend(loss_seq_sa)
    bool_query_seq.extend(bool_query_seq_sa)
    bool_noquery_err_seq.extend(bool_noquery_err_seq_sa)
    categories.extend(['sa'] * len(loss_seq_sa))

    loss_seq_wc, bool_query_seq_wc, bool_noquery_err_seq_wc = analysis_wc_seq(s_wc, g_wc, conf_threshold)
    loss_seq.extend(loss_seq_wc)
    bool_query_seq.extend(bool_query_seq_wc)
    bool_noquery_err_seq.extend(bool_noquery_err_seq_wc)
    categories.extend(['wc'] * len(loss_seq_wc))

    loss_seq_wo, bool_query_seq_wo, bool_noquery_err_seq_wo = analysis_wo_seq(s_wo, g_wn, g_wo, conf_threshold)
    loss_seq.extend(loss_seq_wo)
    bool_query_seq.extend(bool_query_seq_wo)
    bool_noquery_err_seq.extend(bool_noquery_err_seq_wo)
    categories.extend(['wo'] * len(loss_seq_wo))

    loss_seq_wv_se, bool_query_seq_wv_se, bool_noquery_err_seq_wv_se = analysis_wv_se_seq(s_wv, g_wn, g_wvi, conf_threshold)
    loss_seq.extend(loss_seq_wv_se)
    bool_query_seq.extend(bool_query_seq_wv_se)
    bool_noquery_err_seq.extend(bool_noquery_err_seq_wv_se)
    categories.extend(['wv_se'] * len(loss_seq_wv_se))

    return loss_seq, bool_query_seq, bool_noquery_err_seq, categories

def analysis_sc_seq(s_sc, g_sc, conf_threshold):
    logp = F.log_softmax(s_sc, dim=-1)

    loss_seq, bool_query_seq, bool_noquery_err_seq = [], [], []
    for b, g_sc1 in enumerate(g_sc):
        loss_seq.append(-logp[b][g_sc1].item())
        pr_sc1 = torch.Tensor.argmax(logp[b]).item()

        if torch.exp(logp[b][pr_sc1]) < conf_threshold:
            bool_query_seq.append(1)
            bool_noquery_err_seq.append(-1) # invalid
        else:
            bool_query_seq.append(0)
            bool_noquery_err_seq.append(pr_sc1 != g_sc1)

    return loss_seq, bool_query_seq, bool_noquery_err_seq

def analysis_wc_seq(s_wc, g_wc, conf_threshold):
    logp = F.logsigmoid(s_wc)
    loss_seq, bool_query_seq, bool_noquery_err_seq = [], [], []

    for b, g_wc1 in enumerate(g_wc):
        for idx, g_wc11 in enumerate(g_wc1):
            loss_seq.append(-logp[b][g_wc11].item())

        s_wc1 = s_wc[b]

        pr_wc1 = np.argsort(-s_wc1.data.cpu().numpy())[:len(g_wc1)]
        pr_wc1.sort()

        for pr_wc11 in pr_wc1:
            if torch.exp(logp[b][pr_wc11]) < conf_threshold:
                bool_query_seq.append(1)
                bool_noquery_err_seq.append(-1)
            else:
                bool_query_seq.append(0)
                bool_noquery_err_seq.append(pr_wc11 not in g_wc1)

    return loss_seq, bool_query_seq, bool_noquery_err_seq

def analysis_wo_seq(s_wo, g_wn, g_wo, conf_threshold):
    loss_seq, bool_query_seq, bool_noquery_err_seq = [], [], []
    ...
    for b, g_wn1 in enumerate(g_wn):
        ...
        g_wo1 = g_wo[b]
        s_wo1 = s_wo[b]
        logp = F.log_softmax(s_wo1, dim=-1)

        for idx, g_wo11 in enumerate(g_wo1):
            loss_seq.append(-logp[idx][g_wo11].item())

            pr_wo11 = torch.Tensor.argmax(logp[idx]).item()
            if torch.exp(logp[idx][pr_wo11]) < conf_threshold:
                bool_query_seq.append(1)
                bool_noquery_err_seq.append(-1)
            else:
                bool_query_seq.append(0)
                bool_noquery_err_seq.append(pr_wo11 != g_wo11)

    return loss_seq, bool_query_seq, bool_noquery_err_seq

def analysis_wv_se_seq(s_wv, g_wn, g_wvi, conf_threshold):
    """
    s_wv:   [bS, 4, mL, 2], 4 stands for maximum # of condition, 2 tands for start & end logits.
    g_wvi:  [ [1, 3, 2], [4,3] ] (when B=2, wn(b=1) = 3, wn(b=2) = 2).
    """
    ...
    for b, g_wvi1 in enumerate(g_wvi):
        ...
        if g_wn1 == 0:
            continue
        g_wvi1 = np.array(g_wvi1)
        g_st1 = list(g_wvi1[:,0])
        g_ed1 = list(g_wvi1[:,1])

        # loss from the start and end position
        logp_st1 = F.log_softmax(s_wv[b,:g_wn1,:,0], dim=-1)
        logp_ed1 = F.log_softmax(s_wv[b, :g_wn1, :, 1], dim=-1)

        for idx, (g_st11, g_ed11) in enumerate(zip(g_st1, g_ed1)):
            loss_seq.append(-(logp_st1[idx][g_st11] + logp_ed1[idx][g_ed11]).item())
            pr_st11 = torch.Tensor.argmax(logp_st1[idx]).item()
            pr_ed11 = torch.Tensor.argmax(logp_ed1[idx]).item()

            if torch.exp(logp_st1[idx][pr_st11] + logp_ed1[idx][pr_ed11]) < conf_threshold:
                bool_query_seq.append(1)
                bool_noquery_err_seq.append(-1)
            else:
                bool_query_seq.append(0)
                bool_noquery_err_seq.append(pr_st11 != g_st11 or pr_ed11 != g_ed11)
