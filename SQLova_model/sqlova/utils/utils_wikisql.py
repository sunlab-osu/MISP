...
# Wonseok Hwang
#
# Add functions for interaction. @author: Ziyu Yao
...
def get_loader_wikisql_v2(data, bS, shuffle, num_workers=4):
...
def encode(lstm, wemb_l, l, return_hidden=False, hc0=None, last_only=False, dropout_rate=0.0):
    """ [batch_size, max token length, dim_emb]
    """
    # lstm.flatten_parameters()  # TODO: what does it mean? Warning raised with DATA_PARALLEL
    ...
    # dropout
    if dropout_rate > 0.0:
        wenc_mask = torch.FloatTensor(wenc.size()[-1]).view(1, 1, -1)\
            .fill_(1. - dropout_rate).bernoulli().div_(1. - dropout_rate).to(device)
        wenc.data = wenc.data * wenc_mask
...
def encode_hpu(lstm, wemb_hpu, l_hpu, l_hs, dropout_rate=0., mL_hs=None):
    ...
    if mL_hs is None:
        mL_hs = max(l_hs)
    wenc_hs = wenc_hpu.new_zeros(len(l_hs), mL_hs, hS)
...
def update_w2i_wemb(word, wv, idx_w2i, n_total, w2i, wemb):
    """ Follow same approach from SQLNet_model author's code.
...
def pred_wvi_se_beam(max_wn, s_wv, beam_size):
    ...
    for b in range(bS):
        ...
        for i_wn in range(max_wn):
            idxs_st11 = idxs_st1[i_wn] #[k_logit]
...
def pred_wvi_se_interaction_beam(max_wn, s_wv, beam_size, avoid_item_list):
    """
    s_wv: [B, 4, mL, 2]
    - predict best st-idx & ed-idx
...
def punish_len(matrix, actual_lens, device):
    max_len = matrix.size(-1)
    mask = (torch.arange(max_len).expand(matrix.size()) >= actual_lens).float()
    mask = mask.to(device)

    matrix = matrix * (1 - mask) + mask * -10000000000
    return matrix


## Added by Ziyu
def generate_inputs_separate(tokenizer, nlu1_tok, hds1):
    # This function does the same thing as "generate_inputs" but returns the tokens of NL and columns separately.
    tokens_nl, segment_ids_nl = [], []
    tokens_columns, segment_ids_columns = [], []

    # NL
    tokens_nl.append("[CLS]")
    segment_ids_nl.append(0)
    for token in nlu1_tok:
        tokens_nl.append(token)
        segment_ids_nl.append(0)

    # columns
    tokens_columns.append("[SEP]")
    segment_ids_columns.append(0)

    i_hds = []
    # for doc
    for i, hds11 in enumerate(hds1):
        i_st_hd = len(tokens_columns)
        sub_tok = tokenizer.tokenize(hds11)
        tokens_columns += sub_tok
        i_ed_hd = len(tokens_columns)
        i_hds.append((i_st_hd, i_ed_hd))
        segment_ids_columns += [1] * len(sub_tok)
        if i < len(hds1) - 1:
            tokens_columns.append("[SEP]")
            segment_ids_columns.append(0)
        elif i == len(hds1) - 1:
            tokens_columns.append("[SEP]")
            segment_ids_columns.append(1)
        else:
            raise EnvironmentError

    return tokens_nl, segment_ids_nl, tokens_columns, segment_ids_columns, i_hds

def data_preprocessing(tokenizer, data, tables, max_seq_length, bool_remove_none=False, bool_loss_weight=False):
    """
        Here, input is toknized further by WordPiece (WP) tokenizer and fed into BERT.
    ...
    for b, t1 in enumerate(data):
        nlu1, nlu_t1, tid1, sql_i1, sql_q1, sql_t1, tb1, hs_t1, hds1 = get_fields_1(t1, tables, no_hs_t=True, no_sql_t=True)

        g_wvi_corenlp1 = t1['wvi_corenlp']
        ...
        # validation before putting the instance
        try:
            [g_wvi1] = get_g_wvi_bert_from_g_wvi_corenlp([t_to_tt_idx1], [g_wvi_corenlp1])
        except:
            print("Exception in g_wvi1 processing!\nExample: idx {}, t1 {}".format(b, t1))
            exception_ids.append(b)
            continue
        ...
        # ground truths
        g_sc.append(sql_i1["sel"])
        g_sa.append(sql_i1["agg"])
        ...
        # 5. generate l_hpu from i_hds
        l_hpu_batch1 = gen_l_hpu([i_hds1])
        l_hpu_batch.append(l_hpu_batch1)

        # weights
        if "weight_sql" in t1:
            weight_sql1 = t1["weight_sql"]
            weight_sc.append(weight_sql1['sel'])
            weight_sa.append(weight_sql1['agg'])
            weight_wc.append([cond[0] for cond in weight_sql1['conds']])
            # if (weight_wc[-1]).count(0.0) == 0:
            #     weight_wn.append(1.0)
            # else:
            #     weight_wn.append(0.0)
            weight_wn.append(1.0)
            weight_wo.append([cond[1] for cond in weight_sql1['conds']])
            weight_wvi.append([cond[2] for cond in weight_sql1['conds']])
        else:
            weight_sc.append(1.0)
            weight_sa.append(1.0)
            weight_wc.append([1.0] * len(sql_i1['conds']))
            weight_wn.append(1.0)
            weight_wo.append([1.0] * len(sql_i1['conds']))
            weight_wvi.append([1.0] * len(sql_i1['conds']))

    if bool_loss_weight:
        _processed_data = list(zip(input_ids, input_mask, segment_ids, tokens, tb, sql_i, hds,
                                   i_nlu, i_hds, l_n, l_hpu_batch, l_hs, nlu, nlu_t, nlu_tt,
                                   t_to_tt_idx, tt_to_t_idx, g_sc, g_sa, g_wn, g_wc,
                                   g_wo, g_wv, g_wvi, g_wvi_corenlp,
    ...
    if bool_remove_none:
        ...
        print("#of exceptions: %d" % len(exception_ids))
        processed_data = []
        item_idx = 0
        for idx in range(len(data)):
            if idx in exception_ids:
                processed_data.append(None)
            else:
                processed_data.append(_processed_data[item_idx])
                item_idx += 1
        print("processed data size: %d" % len(processed_data))

    return processed_data


def get_wemb_bert_clean(bert_config, model_bert, i_nlu, i_hds, l_n, l_hpu, l_hs,
...
def get_wemb_bert_fast(bert_config, model_bert, i_hds, l_n, l_hpu, l_hs,
                       all_input_ids, all_segment_ids, all_input_mask,
                       num_out_layers_n=1, num_out_layers_h=1):
    bert_outputs = model_bert(all_input_ids, token_type_ids=all_segment_ids, attention_mask=all_input_mask)

    if isinstance(model_bert, AlbertModel):
        _, _, hidden_states = bert_outputs
        all_encoder_layer = hidden_states[1:] # the first one is the embedding output
    else:
        all_encoder_layer, pooled_output = bert_outputs

    # get the wemb
    wemb_n = get_wemb_n_fast(l_n, bert_config.num_hidden_layers, all_encoder_layer, num_out_layers_n)
...
def generate_batched_inputs(input_ids_nl, input_ids_columns, segment_ids_nl, segment_ids_columns, l_n, max_seq_length):
    mL_n = max(l_n)
    ...
    for idx in range(len(input_ids_nl)):
        diff = mL_n - l_n[idx]
        padding_seq = [0] * diff

        source_input_ids_nl = input_ids_nl[idx]
        padded_input_ids1 = source_input_ids_nl + padding_seq + input_ids_columns[idx]

        source_segment_ids_nl = segment_ids_nl[idx]
        padded_segment_ids1 = source_segment_ids_nl + padding_seq + segment_ids_columns[idx]

        input_mask1 = [1] * len(source_input_ids_nl) + padding_seq + [1] * len(input_ids_columns[idx])

        position_ids1 = list(range(max_seq_length - diff))
        position_ids1 = position_ids1[:len(source_input_ids_nl)] + padding_seq + position_ids1[len(source_input_ids_nl):]

        assert len(padded_input_ids1) == len(padded_segment_ids1) == len(input_mask1)

        while len(padded_input_ids1) < max_seq_length:
            padded_input_ids1.append(0)
            padded_segment_ids1.append(0)
            input_mask1.append(0)

        input_ids.append(padded_input_ids1)
        segment_ids.append(padded_segment_ids1)
        input_mask.append(input_mask1)
        position_ids.append(position_ids1)

    return input_ids, input_mask, segment_ids, position_ids


def get_wemb_n_fast(l_n, num_hidden_layers, all_encoder_layer, num_out_layers_n):
    mL_n = max(l_n)

    wemb_n = all_encoder_layer[num_hidden_layers - 1][:, 1:mL_n+1, :]
    num_out_layers_n -= 1

    while num_out_layers_n > 0:
        wemb_n = torch.cat([wemb_n, all_encoder_layer[num_hidden_layers - 1 - num_out_layers_n][:, 1:mL_n+1, :]], -1)
        num_out_layers_n -= 1
