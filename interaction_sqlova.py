...
from SQLova_model.environment import UserSim, RealUser, ErrorEvaluator
...
def real_user_interaction(data_loader, data_table, user, agent, model_bert, bert_config, tokenizer,
    ...
    for iB, t in enumerate(data_loader):
        ...
        # interaction
        hyp, bool_exit = agent.interactive_parsing_session(user, input_item, sql_i[0], init_hyp, bool_verbal=False)
        print("\nPredicted SQL: {}\n".format(hyp.sql))
...
def interaction(data_loader, data_table, user, agent, model_bert, bert_config, tokenizer,
    ...
    for iB, t in enumerate(data_loader):
        ...
        # interaction
        hyp, bool_exit = agent.interactive_parsing_session(user, input_item, sql_i[0], hyp, bool_verbal=False)
...
if __name__ == '__main__':
    ...
    if args.data == "user_study":
        ...
        test_data, test_table = load_wikisql_data(path_wikisql, mode=args.data, toy_model=args.toy_model,
                                                  toy_size=args.toy_size, no_hs_tok=True)
    ...
    world_model = WorldModel((bert_config, model_bert, tokenizer, args.max_seq_length, args.num_target_layers),
                             model, num_options, args.structure, args.seek,
                             num_passes=args.passes, dropout_rate=args.dropout)
    ...
    if args.real_user:
        with torch.no_grad():
            real_user_interaction(test_loader, test_table, user, agent, model_bert, bert_config,
        ...
        with torch.no_grad():
            acc_test, results_test, cnt_list, interaction_records = interaction(
                test_loader, test_table, user, agent, model_bert, bert_config,
                tokenizer, args.max_seq_length, args.num_target_layers,
